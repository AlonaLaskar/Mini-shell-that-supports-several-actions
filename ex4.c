...
void newProcess(char * str, int count) { //Creates a new process if we dont have pips
    char ** a = SplitsTheActions(str);
    pid_t id;
    id = fork();
    if (id < 0) { // error
    ...
    if ((strcmp(str, "fg\n")) == 0){//Check if fg exists and if so allow the program to return to action after it has been suspended
        kill(id,SIGCONT);
        pause();
    }
    if (id == 0) {
    ...
    {
        signal(SIGTSTP,SIG_IGN);//Sends a signal that will suspend the program
        signal(SIGCLD,sig_handler);//Sends a signal every time the son changes his situation
        pause();//father wait until he gets some signal
        deleteTheWords(a,count);
    }
}
...
void creatOnePipe(char * str) { //This function crosses the array before and after the pipe.And sends the commands to execute
    char * token;
    token = strtok(str, "|");//split the array until the "|"
    ...
    if (firstSon == 0) {
        dup2(pipe_onePipd[1], STDOUT_FILENO);//take the input from the pipe
    ...
    if (secondSon == 0) {
        ...
        close(pipe_onePipd[0]);
        close(pipe_onePipd[1]);
        wait(NULL);//wait until the son finished
        wait(NULL);//wait until the son finished
        deleteTheWords(left, num1);//free all the alocation
        deleteTheWords(right, num2);
    }
}
...
void sig_handler(int sigNum){
    waitpid(-1,NULL,WNOHANG);//The wait does not block.Free the resources
}